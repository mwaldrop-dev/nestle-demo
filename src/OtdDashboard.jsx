          ...
          <div style={card}>
            ...
            {OTD_OV_DELAY.map(d => (
              <div key={d.label} style={{ display: "flex", alignItems: "center", marginBottom: 8 }}>
                <div style={{ width: 140, padding: "6px 12px", background: t.surface2, borderRadius: 4, fontSize: 13, color: t.sub, textAlign: "center" }}>{d.label}</div>
                <div style={{ flex: 1, padding: "6px 12px", background: t.surface2, borderRadius: 4, fontSize: 16, fontWeight: 700, color: t.text, textAlign: "center", marginLeft: 8 }}>{d.value}</div>
              </div>
            ))}
          </div>
        </div>
